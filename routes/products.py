...
@router.get(
    "/all",
    response_model=List[ProductAllResponse],
...
async def get_all_products_admin(
    ...
    try:
        ...
        if barcode:
            ...
            if (
                barcode_clean.isdigit()
                and len(barcode_clean) in [12, 13]
                and barcode_clean.startswith("20")
            ):
                try:
        ...
        if provider_code:
            ...
            query += (
                " LEFT JOIN warehouse_stock_variants wsv_p ON wsv_p.product_id = p.id"
            )
        ...
        if search:
            ...
            if (
                search_clean.isdigit()
                and len(search_clean) in [12, 13]
                and search_clean.startswith("20")
            ):
                try:
        ...
        products = (
            await db.fetch_all(query, *params) if params else await db.fetch_all(query)
        )
...
@router.get(
    "/allProductsInfo/{product_id}",
    response_model=ProductInfoMatrix,
    dependencies=[Depends(require_admin)],
)
    ...
    try:
        ...
        if toggle_data.en_tienda_online:
            ...
            existing_slug = await db.fetch_one(
                "SELECT id FROM products WHERE slug = $1 AND id != $2", slug, product_id
            )
            if existing_slug:
                raise HTTPException(
        ...
        if toggle_data.tags is not None:
            ...
            await db.execute(
                "DELETE FROM product_tags WHERE product_id = $1", product_id
            )
...
async def update_product_web_price(product_id: int, precio_web: float):
    ...
    try:
        # Check if product exists
        existing = await db.fetch_one(
            "SELECT id, nombre_web FROM products WHERE id = $1", product_id
        )
        if not existing:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
async def apply_product_discount(
    ...
    try:
        ...
        product = await db.fetch_one(
            "SELECT id, sale_price FROM products WHERE id = $1", product_id
        )
        if not product:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        if has_discount == 1:
            ...
            return {
                ...
                "product_name": product.get(
                    "product_name", ""
                ),  # Might not be fetched in validation query, but simplified return
                "sale_price": product["sale_price"],
...
async def add_product_image(
    product_id: int, image: ImageUpload, current_user: dict = Depends(require_admin)
):
    """
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Imagen base64 inválida"
            )
        ...
        result = await db.fetch_one(
            ...
            product_id,
            b"",
            orden,  # Empty bytes for legacy BLOB column
        )
...
async def delete_product_image(
    product_id: int, image_id: int, current_user: dict = Depends(require_admin)
):
    """
...
async def update_product(product_id: int, payload: ProductoUpdateSchema):
    try:
            ...
            if payload.variantes:
                for var_input in payload.variantes:
                    ...
                    if not variant_exists:
                        logger.warning(
                            f"Variante {var_input.id} no encontrada para producto {product_id}, saltando..."
                        )
                        continue
        ...
        logger.info(
            f"Producto {product_id} actualizado correctamente con {len(payload.variantes)} variantes"
        )
...
async def get_all_productos(
    ...
    try:
        ...
        if branch_id is None:
            ...
            branch_exists = await db.fetchval(
                "SELECT id FROM storage WHERE id = $1", branch_id
            )
...
async def filter_online_products(
    tags: List[str] = Query(default=[]), groupName: Optional[str] = None
):
    """
    ...
    try:
        ...
        if groupName and group_ids:
            ...
            group_placeholders = ", ".join(
                [f"${i}" for i in range(start_idx, start_idx + len(group_ids))]
            )
            ...
            product_dict["variantes"] = [
                {
                    ...
                    "barcode": v[
                        "barcode"
                    ],  # Queda vacío como en tu endpoint original web
                }
                for v in variants
            ]
...
async def search_products_online(
    nombre_web: str = Query(..., description="Término de búsqueda (por nombre web)"),
):
    """
...
async def get_product(
    product_id: int,
    branch_id: Optional[int] = Query(
        None, description="ID de sucursal. Null = Stock Web manual."
    ),
):
    """
...
async def get_online_store_products(
    category: Optional[str] = None, limit: Optional[int] = 50, offset: Optional[int] = 0
):
    """
    ...
    try:
        ...
        query_product = """
            SELECT 
                ...
                -- Stock total web disponible (solo variantes web activas)
...
async def debug_barcode_lookup(barcode: str):
    ...
    try:
        ...
        query_exact = (
            "SELECT * FROM warehouse_stock_variants WHERE variant_barcode = $1"
        )
        exact_matches = await db.fetch_all(query_exact, barcode)

        # 2. Case-insensitive match in warehouse_stock_variants
        query_ilike = (
            "SELECT * FROM warehouse_stock_variants WHERE variant_barcode ILIKE $1"
        )
